...
class TasksTabTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Here, we initiate the app environment, and we mock the data from the
        gazu API needed by the app
        """
        ...
        gazu.user.all_tasks_to_do = \
            mock.MagicMock(return_value=cls.mock_data.tasks())
        gazu.task.all_task_statuses = \
            mock.MagicMock(return_value=cls.mock_data.status_names())

    def test_wrong_attribute(self):
        """
        Test the behaviour of the class when asked for an inexisting task
        attribute.
        """
    ...
    def test_creation(self):
        ...
        for row in range(0, header_row_count):
            for col in range(0, header_col_count-1):
                assert(
                    isinstance(tasks_table.item(row, col),
                    QtWidgets.QTableWidgetItem)
                )
            assert(
                isinstance(tasks_table.cellWidget(row, header_col_count-1),
